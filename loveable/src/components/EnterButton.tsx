...
export default function EnterButton({ 
  ...
  return (
    <motion.div 
      ...
      transition={{ duration: 1, delay: isZooming ? 0 : 1.6, ease: [0.16, 1, 0.3, 1] }}
    >
      <motion.button
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-center gap-1">
            <span 
              className={`
                ...
                ${isHovered ? 'text-primary-foreground' : 'text-foreground/80'}
              `}
            >
              ENTER
            </span>
            <div className={`w-4 h-px transition-all duration-500 ${isHovered ? 'bg-primary-foreground/50 w-6' : 'bg-foreground/30'}`} />
            <span 
              className={`
                terminal-text text-[9px] md:text-[10px] tracking-[0.2em] font-medium
                transition-colors duration-500
          ...
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
            <motion.circle
              ...
              r="48"
              fill="none"
              stroke={isHovered ? 'hsl(var(--primary-foreground) / 0.3)' : 'hsl(var(--foreground) / 0.1)'}
              strokeWidth="0.5"
              strokeDasharray="4 8"
        ...
        {/* Label below */}
        <motion.span 
          className="absolute -bottom-8 left-1/2 -translate-x-1/2 terminal-text text-muted-foreground text-[8px] tracking-[0.15em] whitespace-nowrap"
