...
function EarthWireframe({ isPulsing, mousePosition }) {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.rotation.x +=
        (targetRotationX - meshRef.current.rotation.x) * 0.03;
      meshRef.current.rotation.z +=
        (targetRotationZ - meshRef.current.rotation.z) * 0.03;
    ...
    if (glowRef.current) {
      glowRef.current.material.opacity = isPulsing
...
export default function Earth3D({ isPulsing, mousePosition, isZooming }) {
  return (
    <motion.div
      ...
      transition={{
        opacity: {
          duration: isZooming ? 1.2 : 1.5,
          delay: isZooming ? 0 : 0.3,
        },
        scale: {
          duration: isZooming ? 1.2 : 1.5,
          ease: [0.16, 1, 0.3, 1],
